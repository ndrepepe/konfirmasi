...
export function CustomerExcelImporter({ branches }: { branches: Branch[] }) {
  ...
  async function handleImport(formData: FormData) {
    ...
    try {
      ...
      if (!normalizedRows.length) throw new Error("Tidak ada data customer yang bisa diimport.");
      ...
      if (missingBranches.length) {
        throw new Error(`Kode cabang tidak ditemukan: ${missingBranches.join(", ")}`);
      }

      const parsed = normalizedRows.map((row, index) => {
        if (!row.customer_code) throw new Error(`ID Customer kosong pada baris ${index + 2}.`);
        if (!row.branch_code) throw new Error(`Kode Cabang kosong pada baris ${index + 2}.`);
        if (!row.customer_name) throw new Error(`Nama Customer kosong pada baris ${index + 2}.`);
      ...
      setStatus("uploading");
      setMessage("Mengupload data ke Supabase...");
      ...
      setStatus("done");
      setMessage(`${parsed.length.toLocaleString("id-ID")} data customer berhasil diimport.`);
      router.refresh();
    } catch (error) {
      setStatus("error");
